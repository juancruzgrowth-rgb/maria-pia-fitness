...
1. **Branding obligatorio:** paleta MP (`mp.canvas`, `mp.ink`, `mp.carbon`, `mp.orange`, `mp.amber`, `mp.sky`, `mp.line`). Tipografía `font-display` (Fraunces) para titulares, `font-sans` (Newsreader) para texto y `font-ui` (Montserrat) para etiquetas y CTAs.
